...
function contactListHandler() {
  document.getElementById('contact-list-wrapper').addEventListener('click', (event) => {
    clickOnCard(event);
  });
  document.getElementById('contact-list-wrapper').addEventListener('click', (event) => {
...
function renderContacts() {
  ...
  for (let i = 0; i < groupedUsers.length; i++) {
    addIntoContainer('contact-list-wrapper', `<h4 class="contact-list-divider outside-trigger">${groupedUsers[i]['group']}</h4>`);
...
function contactCardTemplate(user, i, j) {
  ...
  <label for="contact-btn-${i}-${j}" class="contact-wrapper">
      <div id="user-icon-${i}-${j}" class="user-icon" ${colorContactIcon(user)}>${initialLetter(user, 0)}${initialLetter(user, user.name.search(' ') + 1)}</div>
...
function contactDetailsTemplate(user, i, j) {
  ...
  <div class="contact-wrapper">
    <div id="user-icon-${i}-${j}" class="user-icon user-icon-big" ${colorContactIcon(user)}>${initialLetter(user, 0)}${initialLetter(user, user.name.search(' ') + 1)}</div>
...
function colorContactIcon(user) {
  return `style="background-color:` + user.color + `; color:` + responsiveColor(user.color) + `";`;
}
